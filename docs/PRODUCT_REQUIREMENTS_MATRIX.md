...
Remediation update: 2026-08-17 after web review loop sprint
...
| Immutable original evidence                             | VERIFIED | Local originals are copied and hashed; mobile uploads originals to private storage and completion verifies object size/type/hash before accepting.                                       |
...
| Private object storage                                  | PARTIAL  | Evidence originals use authenticated signed upload/download and server object verification. Cloud report PDFs are not archived there yet.                                                |
...
| Web reports view                                        | PARTIAL  | Reports page and detail pages read tenant-scoped report drafts and readiness. Cloud PDF download/share links are missing.                                                                |
...
| Reconnect                              | PARTIAL  | Manual metadata and media upload work with native Clerk auth; pull sync/background reconciliation are missing.                              |
| Verify sync                            | PARTIAL  | Server receives/applies metadata, verifies uploaded media originals, and exposes tenant web review. Pull sync/conflicts remain future work. |
...
| Download previous report in web        | PARTIAL  | Web report detail exists, but cloud PDF storage/download is not implemented.                                                                |
