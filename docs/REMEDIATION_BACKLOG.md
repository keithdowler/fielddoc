...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------- |
| Proof Packet output    | Local Proof Packet PDFs embed available local image originals with captions/metadata and include imported document metadata appendix cards plus scanned page hashes.                                                                                                                                                                 | Imported PDF page rasterization, visual QA, cloud report versions.              |
| Capture speed          | Mobile Capture has sticky before/work/after stage controls, quick captioning, next-stage control, batch photo import, and multi-page document scan grouping.                                                                                                                                                                         | Camera loop polish, retake/replace history, native scanner crop mode.           |
...
| Provider verification  | Web has private authenticated operational test endpoints for Resend email delivery and Sentry server error reporting, with tenant-scoped audit rows for successful checks.                                                                                                                                                           | Real deliverability, Sentry alert routing, and mobile crash reporting.          |
...
| -------- | ------------ | ------------------------------------------------------------------------------------------- | ---------- | ----------- | ---------------- | ------------------- |
| P1       | CORE VALUE   | Render imported PDF pages or thumbnails into Proof Packet PDFs                              | M          | Yes         | Yes              | Yes                 |
...
| P2       | PRODUCTION     | Add mobile Sentry and privacy-safe analytics event taxonomy                             | S          | No          | No               | No                  |
...
| P2       | PRODUCTION     | Verify Resend deliverability, Sentry alert routing, privacy URL, and terms URL          | S          | No          | Yes              | No                  |
...
high-value actions, has RevenueCat entitlement enforcement, preserves imported
documents as external originals in local Proof Packets, and presents public
share links as branded delivery pages. The largest remaining core-value blocker
is rendering imported PDFs visually while keeping a clear security boundary for
untrusted document content.

### Scope

1. Generate safe imported PDF page previews or thumbnails.
2. Render imported PDF previews into Proof Packet output when available.
3. Add quarantine/scanning status fields for uploaded documents before download.
4. Preserve immutable originals and derivatives with clear source metadata.
5. Add document-focused tests across SQLite, report assembly, and PDF output.
6. Keep OCR and advanced document search out of scope until the document loop is
   stable.
